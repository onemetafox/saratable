    ...
    <div class="card-body">
      ...
      <form class="geniusform" action="{{ route('admin.gs.update') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
            ...
            <div class="col-md-4">
                <div class="form-group">
                  <label for="inp-title">{{  __('Enable Force SSL Redirection')  }}</label>
                  <div class="frm-btn btn-group mb-1">
                      <button type="button" class="btn btn-sm btn-rounded dropdown-toggle btn-{{ $gs->is_ssl == 1 ? 'success' : 'danger' }}" data-toggle="dropdown"
                        aria-haspopup="true" aria-expanded="false">
                        {{ $gs->is_ssl == 1 ? __('Activated') : __('Deactivated')}}
                      </button>
                      <div class="dropdown-menu">
                        <a class="dropdown-item drop-change" href="javascript:;" data-status="1" data-val="{{ __('Activated') }}" data-href="{{ route('admin.gs.status',['is_ssl',1]) }}">{{ __('Activate') }}</a>
                        <a class="dropdown-item drop-change" href="javascript:;" data-status="0" data-val="{{ __('Deactivated') }}" data-href="{{ route('admin.gs.status',['is_ssl',0]) }}">{{ __('Deactivate') }}</a>
                      </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
          <div class="col-md-6">
            ...
            <div class="col-md-12">
                <div class="form-group">
                  <label for="inp-name">{{ __('Time Zone') }}</label>
                  @php
            ...
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="inp-phone_code" name="talkto"  placeholder="{{ __('Enter phone code') }}" value="{{ $gs->talkto }}" required>
              </div>
            </div>
