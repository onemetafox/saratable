...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <form class="geniusform" action="{{ route('admin.profile.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="inp-website">{{ __('Website') }}</label>
                <input type="text" class="form-control" id="inp-website" name="website" placeholder="{{ __('Enter Website') }}" value="{{ $data->website }}" required>
            </div>

            <div class="form-group">
                <label for="inp-direction">{{ __('Direction') }}</label>
                <input type="text" class="form-control" id="inp-direction" name="direction" placeholder="{{ __('Enter Direction') }}" value="{{ $data->direction }}" required>
            </div>
