      ...
      <div class="card-body py-5">
        ...
        <form class="geniusform" action="{{route('admin.plans.update',$data->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control" id="price" name="price" placeholder="{{ __('Plan Price') }}" min="0" step="0.01" value="{{ showBladeAdminAmount($data->price) }}">
            </div>

            <div class="form-group">
                <label for="prev_price">{{ __('Previous Price') }} ({{$currency->name}})</label>
                <input type="number" class="form-control" id="prev_price" name="prev_price" placeholder="{{ __('Plan Previous Price') }}" min="0" step="0.01" value="{{ showBladeAdminAmount($data->prev_price) }}">
                <small class="text-muted">@lang('Leave 0 if you do not want to set.')</small>
            </div>
