            ...
            <div class="dashboard-tlbar d-block mb-5">
                <div class="row">
                    <div class="colxl-12 col-lg-12 col-md-12">
                        <h1 class="ft-medium">@lang('Beauty Booking Request')</h1>
                                ...
                                <div class="dashboard-bookings-wraps">
                                    @foreach ($beauties as $key=>$beauty)
                                        ...
                                        <div class="dsd-single-bookings-wraps">
                                            <div class="dsd-single-book-thumb"><img src="{{ asset('assets/images/'.$beauty->user->photo) }}" class="img-fluid circle" alt="" /></div>
                                            <div class="dsd-single-book-caption">
                                                <div class="dsd-single-book-title"><h5>{{ $beauty->user->name }}<span class="bko-dates">{{ $beauty->user->created_at->format('d M Y') }}</span></h5></div>
                                                <div class="ico-content">
                                                    <ul>
                                                        <li>
                                                            <div class="px-2 py-1 medium bg-light-{{ $beauty->status == 0 ? 'warning' :($beauty->status == 1 ? 'success':'danger')}} rounded text-{{ $beauty->status == 0 ? 'danger' :($beauty->status == 1 ? 'success':'primary')}}">
                                                                @if ($beauty->status == 0)
                                                                    @lang('Pending')
                                                                @elseif($beauty->status == 1)
                                                                    @lang('Approved')
                                                                @else
                                                                    @lang('Rejected')
                                                                @endif
                                                ...
                                                <div class="dsd-single-descr">
                                                    ...
                                                    <div class="dsd-single-item">
                                                        ...
                                                        <span class="dsd-item-info">{{ CArbon\Carbon::parse($beauty->checkin_date)->format('d M Y')}}</span>
                                                    </div>

                                                    <div class="dsd-single-item">
                                                        <span class="dsd-item-title">@lang('Service Name'):</span>
                                                        <span class="dsd-item-info">{{ $beauty->service->service_name }}</span>
                                                    </div>

                                                    @if ($beauty->note)
                                                        <div class="dsd-single-item">
                                                            <span class="dsd-item-title">@lang('Note'):</span>
                                                            <span class="dsd-item-info">{{ $beauty->note }}</span>
                                                        </div>
                                                    @endif

                                                    <div class="dsd-single-item">
                                                        <span class="dsd-item-title">@lang('Member'):</span>
                                                        <span class="dsd-item-info">{{ $beauty->adults }} @lang('Adults'), {{ $beauty->kids }} @lang('Child')</span>
                                                    </div>

                                                    <div class="dsd-single-item">
                                                        <span class="dsd-item-title">@lang('Mail'):</span>
                                                        <span class="dsd-item-info">{{ $beauty->user->email }}</span>
                                                    </div>

                                                    <div class="dsd-single-item">
                                                        <span class="dsd-item-title">@lang('Phone'):</span>
                                                        <span class="dsd-item-info">{{ $beauty->user->phone }}</span>
                                                    </div>
                                                </div>
                                                <div class="dsd-single-book-footer">
                                                    @if ($beauty->status == 0)
                                                        <a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#confirm-status" data-href="{{ route('user.booking.status',['id1' => $beauty->id, 'id2' => 1]) }}" class="btn btn-aprd">
                                                            <i class="fas fa-check-circle me-1"></i>
                                                            @lang('Approved')
                                                        </a>

                                                        <a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#confirm-status" data-href="{{ route('user.booking.status',['id1' => $beauty->id, 'id2' => 2]) }}" class="btn btn-reject">
                                                            <i class="fas fa-trash me-1"></i>
                                                            @lang('Reject')
                                                        </a>
                                                    @endif
                                                    <a href="{{ route('user.booking.conversation',$beauty->id) }}" class="btn btn-message"><i class="fas fa-envelope me-1"></i>@lang('Message')</a>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
