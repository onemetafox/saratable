(function($) {
  ...
  $('.dynamic-modal .btn-ok').on('click', function(e) {
      $.ajax({
      type:"GET",
      url:$(this).attr('href'),
      success:function(data)
...
$(document).ready(function () {
                  ...
                  <div class="card-body">
                      <div class="media">
                          <div class="media-body">
                              ...
                              <input type="hidden" name="${keyword}[dropdown]" value="${dropdown}">
                              <input type="hidden" name="${keyword}[href]" value="${href}">
                              <input type="hidden" name="${keyword}[parent]" value="${parent}">
                              <input type="hidden" name="${keyword}[target]" value="${target}">
                          </div>
                          <i class="remove-menu fa fa-trash-alt"></i>
                      </div>
  ...
  $(document).on('click', '#property_gallery', function () {
	...
	$("#propertyuploadgallery").change(function (event) {
		var total_file = document.getElementById("propertyuploadgallery").files.length;
		for (var i = 0; i < total_file; i++) {
			$('.selected-image .row').append('<div class="col-sm-6">' +
				...
				'<img class="back-preview-imag" src="' + URL.createObjectURL(event.target.files[i]) + '" alt="gallery image">' +
				'</a>' +
				'</div>' +
				'</div> '
			);
			$('#geniusform').append('<input type="hidden" name="gallery[]" id="galval' + i +
